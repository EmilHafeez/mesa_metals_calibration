...
Then we look at these regression lines. Each unique calibration run is its own regression line, as in the Excel structure. Here, we just do the newer arsenic data (and then we'll look at all old and new data for all elements later).
...
Focusing on these regression lines based on the newer arsenic data, let's take a look at the intercepts against the slopes. 
```{r, echo = F}
As_new_results_df %>% 
...
I read in the data, then, iterate the regressions for each of the (date,no_cal) groups within each element.
```{r, include = F}
path =
  "./data/210429_MESA totals calibration comparison_for Jeff and Emil.xlsx"
...
(patchwork_new_calibrations + patchwork_old_calibrations) + 
...
```{r, echo = F, message = F, warning = F, include = F, fig.fullwidth=TRUE}
#plot it
# mesa_metals_results_df %>% 
...
#     x = intercept, 
#     y = beta_hat_combined, 
#     group = no_cal_combined, 
#     color = no_cal_combined,
#     shape = no_cal_combined
#     )
#   ) +
#   geom_point(size = 3) + 
...
#   facet_wrap(~elements + old_new_identifier, scales = "free") +
#     scale_shape_manual(values=seq(15,23)) + 
#   theme_bw(base_size = 8)
```

```{r, echo = F, message = F, warning = F, include = T, fig.fullwidth=TRUE}
