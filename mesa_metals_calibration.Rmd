...
```{r, message = F, warning = F, echo = F, fig.width= 30, fig.height = 25.02}
mesa_metals_df %>% 
  mutate(date = as.factor(date)) %>% 
ggplot(
  aes(
...
Like we did for arsenic, let's look at the intercepts vs slopes for these regression lines, per element also. 
...
```{r, echo = F, message = F, warning = F, fig.width= 25.37, fig.height = 19.02}
#plot it
mesa_metals_results_df %>% 
